...
> 版本: v1.0 | 创建日期: 2026-05-01 | 内容: 因子计算公式与实现依据
...
### 3.1 板块持续性 (Persistence)

**原理**: 衡量板块在排名中的稳定性。

**公式**:
```
排名历史 = [rank_day1, rank_day2, ..., rank_dayN]
top_k = 10  # 关注前10名

持续性 = count(rank in top_k for rank in rank_history) / len(rank_history)
```

**评分逻辑**:
```
持续性 > 0.7  → 稳定强势 → 得分 8-10
持续性 0.4-0.7 → 偶尔强势 → 得分 4-7
持续性 < 0.4  → 不稳定 → 得分 0-4
```
