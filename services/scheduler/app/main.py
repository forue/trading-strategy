    ...
    try:
        async with httpx.AsyncClient(timeout=120) as client:
            for strategy in ["AGGRESSIVE", "MODERATE", "CONSERVATIVE"]:
                resp = await client.post(f"{settings.strategy_url}/calculate?strategy_type={strategy}")
